...
class Reviewer(BaseModel):
    ...
    review: str = Field(None, description="Specialisation of the reviewer")
    suggestions: str = Field(None, description="Specialisation of the reviewer")


class QAPair(BaseModel):
    query: str = Field(None, description="Query string")
    answer: bool = Field(None, description="Answer string")
...
class Paper(BaseModel):
    filepath: str = Field(None, description="Filepath of the paper")
...
class FRPair(BaseModel):
    ...
    k_search_query: str = Field(
        default="", description="Query used for knowledge search"
    )
    search_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="Results from web search"
    )
    k_search_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="Results from knowledge search"
    )
    verification: Dict[str, Any] = Field(
        default_factory=dict, description="Verification results with scores"
    )
    k_verification: Dict[str, Any] = Field(
        default_factory=dict,
        description="Verification results with scores from knowledge search",
    )
